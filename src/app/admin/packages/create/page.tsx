...
export default function CreatePackage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Paket Sewa berhasil ditambahkan!");
  ...
  return (
    <div style={{ maxWidth: "800px", margin: "0 auto", paddingBottom: "40px" }}>
      <div style={{ marginBottom: "24px", display: "flex", alignItems: "center", gap: "16px" }}>
        ...
        <div>
          <h1 style={{ fontSize: "1.8rem", fontWeight: 800, margin: 0 }}>Tambah Paket Sewa</h1>
          <p style={{ color: "var(--foreground-muted)", fontSize: "0.95rem", margin: 0 }}>Buat bundel produk dengan harga spesial.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div className="responsive-grid-2">
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={inputStyle} placeholder="Misal: Paket Camping Berdua" />
            </div>

            <div style={{ gridColumn: "1 / -1" }}>
              <label style={labelStyle}>Deskripsi Paket *</label>
              <textarea required value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} style={{ ...inputStyle, minHeight: "80px", resize: "vertical" }} placeholder="Deskripsi lengkap paket..." />
            </div>
            
            <div style={{ gridColumn: "1 / -1" }}>
              <label style={labelStyle}>Foto Paket (Maks 2MB, JPG/PNG)</label>
          ...
          <div style={{ display: "flex", gap: "16px", marginTop: "16px", justifyContent: "flex-end" }}>
            ...
            <button type="submit" disabled={saving || items.length === 0} className="btn-primary" style={{ opacity: (saving || items.length === 0) ? 0.7 : 1 }}>
              {saving ? "⏳ Menyimpan..." : "💾 Simpan Paket Sewa"}
            </button>
          </div>
        </form>
      </div>
