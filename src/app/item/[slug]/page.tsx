  ...
  const handleAddToCart = (redirect = false) => {
    ...
    if (existingIndex >= 0) {
      ...
      cart.push({
        ...
        image_url: product.images?.[0]?.image_path ? (product.images[0].image_path.startsWith('http') ? product.images[0].image_path : 'http://localhost:8000' + product.images[0].image_path) : null,
      ...
      {showLightbox && product.images && product.images.length > 0 && (
        <div className="animate-fade-in" style={{
          ...
          <div style={{ position: "relative", width: "90vw", height: "80vh", maxWidth: "1000px" }}>
            <Image
              src={product.images[currentImageIndex].image_path.startsWith('http') ? product.images[currentImageIndex].image_path : 'http://localhost:8000' + product.images[currentImageIndex].image_path}
              alt={product.name}
              fill
          ...
          {product.images && product.images.length > 1 && (
            <div style={{ display: "flex", gap: "12px", overflowX: "auto", paddingBottom: "12px", marginBottom: "16px" }} className="hide-scrollbar">
              {product.images.map((img, index) => (
                ...
                >
                  <Image src={img.image_path.startsWith('http') ? img.image_path : 'http://localhost:8000' + img.image_path} alt={`Thumbnail ${index + 1}`} fill style={{ objectFit: 'cover' }} unoptimized />
                </button>
              ))}
            </div>
          )}
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {review.photo_url && (
                              <div style={{ marginTop: "16px", borderRadius: "var(--radius-md)", overflow: "hidden", border: "1px solid var(--border)", display: "inline-block" }}>
                                <Image 
                                  src={review.photo_url.startsWith('http') ? review.photo_url : `http://localhost:8000${review.photo_url}`} 
                                  alt="Foto Ulasan" 
                                  width={120} height={120} 
                                  style={{ objectFit: "cover", cursor: "pointer", transition: "transform 0.2s" }} 
                                  onMouseEnter={(e) => e.currentTarget.style.transform = "scale(1.05)"}
                                  onMouseLeave={(e) => e.currentTarget.style.transform = "scale(1)"}
                                  onClick={() => window.open(review.photo_url?.startsWith('http') ? review.photo_url : `http://localhost:8000${review.photo_url}`, '_blank')}
                                  unoptimized
                                />
                              </div>
                            )}
            ...
            {relatedProducts.map((rp) => (
              <Link key={rp.id} href={`/item/${rp.slug}`} className="card flex flex-col hover-scale" style={{ textDecoration: "none", color: "var(--foreground)", overflow: "hidden" }}>
                <div style={{ height: "140px", background: "var(--primary-gradient)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                  {rp.primary_image ? (
                    <Image src={rp.primary_image.image_path.startsWith('http') ? rp.primary_image.image_path : 'http://localhost:8000' + rp.primary_image.image_path} alt={rp.name} fill style={{ objectFit: 'cover' }} unoptimized />
                  ) : (
